...
it("delivers group claude connections and the automatically scheduled agent model to execution", async () => {
  ...
  try {
    ...
    for (const version of [1, 2]) {
      const config = { profile: { name: "private", base_url: `http://127.0.0.1:${8100 + version}/v1`, model: "default-model", models: ["default-model", "selected-1", "selected-2"], env_key: "", auth_mode: "api_key", auth_header: version === 1 ? "bearer" : "x-api-key" }, api_key: `private-key-${version}` };
