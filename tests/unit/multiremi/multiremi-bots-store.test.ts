...
describe("Bot configuration and durable conversation routing", () => {
  ...
  it("preserves message order and the latest preview across Bot turns", () => {
    const { store, send } = fixture();
    const first = send("first", { text: "First question" });
    expect(store.claimTask("executor")?.id).toBe(first.taskId);
    store.startTask(first.taskId);
    store.completeTask(first.taskId, { output: "First answer" });
    send("second", { text: "Follow-up question" });
    const messages = store.listChatMessages(first.chatSessionId);
    expect(messages.map((message) => message.body)).toEqual(["First question", "First answer", "Follow-up question"]);
    expect(store.listChatSessions("local", { creatorId: "viewer" })[0]?.lastMessage?.content).toBe("Follow-up question");
  });

  it("waits for the same account on another Bot to stop before takeover", () => {
    ...
    store.updateBot(bot.id, { ...input, enabled: false });
    const next = store.createBot({ ...input, name: "Replacement Bot", platform_bindings: [
      { ...input.platform_bindings[0]!, id: "next-account", host_runtime_id: "replacement" },
    ] });
    expect(store.getBotDaemonAssignment(next.id, "next-account", "replacement")).toBeNull();
    store.reportBotRuntimeStatus(bot.id, "account-a", "host", { appliedRevision: bot.revision, state: "stopped" });
    expect(store.getBotDaemonAssignment(next.id, "next-account", "replacement")?.app_id).toBe("app_a");
  });
  ...
  it("allows pausing and clearing the original binding after its host is deleted", () => {
    const { store, bot, input } = fixture();
    // Runtime deletion keeps one Runtime per managed daemon; retain a sibling.
    store.registerRuntime({ id: "host-sibling", name: "Host sibling", provider: "claude-code", workspaceId: "local", daemonId: "daemon-host" });
    expect(store.deleteRuntime("host")).toBe(true);
    const disabled: SaveBotInput = { ...input, enabled: false,
      platform_bindings: input.platform_bindings.map((binding) => ({ ...binding, app_secret_op: "clear", app_secret: undefined })),
    };
    const saved = store.updateBot(bot.id, disabled);
    expect(saved.enabled).toBe(false);
    expect(saved.platform_bindings.every((binding) => !binding.app_secret_configured)).toBe(true);
    expect(store.botDirectivesForRuntime("host").every((directive) => directive.desired_state === "stopped")).toBe(true);
    expect(() => store.updateBot(bot.id, input)).toThrow("Platform host belongs to another workspace");
    expect(() => store.updateBot(bot.id, { ...disabled,
      platform_bindings: [{ ...input.platform_bindings[0]!, id: "new-binding" }],
    })).toThrow("Platform host belongs to another workspace");
    const remote = store.createWorkspace({ name: "Other host space", slug: "other-host-space" });
    store.registerRuntime({ id: "remote-host", name: "Remote host", provider: "codex", workspaceId: remote.id });
    expect(() => store.updateBot(bot.id, { ...disabled,
      platform_bindings: [{ ...input.platform_bindings[0]!, host_runtime_id: "remote-host" }],
    })).toThrow("Platform host belongs to another workspace");
    expect(store.getBot(bot.id)?.enabled).toBe(false);
  });

  it("allows pausing and clearing credentials after an existing target is archived", () => {
    const { store, bot, input, agent } = fixture();
    const configured: SaveBotInput = { ...input,
      routes: [{ id: "same", name: "Same Agent", match: {}, target: input.default_target }],
      issue_notifications: { platform_binding_id: "account-a", chat_id: "updates", target: input.default_target },
    };
    store.updateBot(bot.id, configured);
    store.archiveAgent(agent.id);
    const disabled: SaveBotInput = { ...configured, enabled: false,
      platform_bindings: configured.platform_bindings.map((binding) => ({ ...binding, app_secret_op: "clear", app_secret: undefined })),
    };
    const saved = store.updateBot(bot.id, disabled);
    expect(saved.enabled).toBe(false);
    expect(saved.platform_bindings.every((binding) => !binding.app_secret_configured)).toBe(true);
    expect(store.botDirectivesForRuntime("host").every((directive) => directive.desired_state === "stopped")).toBe(true);
    expect(store.getBotDaemonAssignment(bot.id, "account-a", "host")).toBeNull();
    expect(() => store.updateBot(bot.id, configured)).toThrow("Target Agent is unavailable");
    // Leaving the Bot enabled must still validate targets even if a binding is disabled.
    expect(() => store.updateBot(bot.id, { ...configured,
      platform_bindings: configured.platform_bindings.map((binding) => ({ ...binding, enabled: false })),
    })).toThrow("Target Agent is unavailable");
    const remoteWorkspace = store.createWorkspace({ name: "Other space", slug: "other-space" });
    const remoteAgent = store.createAgent({ name: "Other space Agent", provider: "codex", workspaceId: remoteWorkspace.id });
    const remoteTarget = { kind: "agent" as const, agent_id: remoteAgent.id };
    expect(() => store.updateBot(bot.id, { ...disabled, default_target: remoteTarget })).toThrow("Target Agent is unavailable");
    expect(() => store.updateBot(bot.id, { ...disabled,
      routes: [{ id: "same", name: "Remote route", match: {}, target: remoteTarget }],
    })).toThrow("Target Agent is unavailable");
    expect(() => store.updateBot(bot.id, { ...disabled,
      issue_notifications: { platform_binding_id: "account-a", chat_id: "updates", target: remoteTarget },
    })).toThrow("Target Agent is unavailable");
    expect(store.getBot(bot.id)?.enabled).toBe(false);
  });
