...
title: Chat 私聊与消息队列
status: active
summary: 独立 Chat 页面与浮窗共用私聊、工作位置、执行队列和会话管理。
---
...
新聊天选择云友，首条消息或首次附件上传时创建会话。工作位置沿用
[Runtime 工作区契约](dev/runtime-workspaces.md)：项目和本机目录互斥；会话创建后工作位置固定，
换位置需新建聊天。项目背景和仓库沿用既有执行上下文，本机目录仍只在所属 Runtime 上执行。
...
Web 的 `X-Workspace-Slug` 用于解析聊天列表、创建和待处理任务的工作区；未知 slug 不回退到 `local`，
与显式工作区参数冲突时拒绝请求。
...
```bash
remi chat create --agent <id> --project <id>
...
验证方法遵循[测试指南](../TESTING.md)；单元测试、浏览器冒烟和真实 provider 执行分别报告结果。
