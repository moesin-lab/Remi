...
describe("Chat queues", () => {
  ...
  it("migrates legacy Chat order once and continues the sequence after reopening", () => {
    ...
    db!.run("INSERT INTO multiremi_chat_messages (id, chat_session_id, role, body, created_at) VALUES (?, ?, 'assistant', 'legacy answer', ?)", ["msg_a_legacy", chat.id, "2026-01-01T00:00:00.000Z"]);
    db!.run("DELETE FROM multiremi_schema_migrations WHERE id = '20260911_chat_message_sequence'");
    db!.run("DROP INDEX idx_multiremi_chat_messages_sequence");
    db!.run("ALTER TABLE multiremi_chat_messages DROP COLUMN message_seq");
    db!.run("ALTER TABLE multiremi_chat_sessions DROP COLUMN next_message_seq");
    ...
    expect(db!.query("SELECT message_seq FROM multiremi_chat_messages WHERE chat_session_id = ? ORDER BY message_seq").all(chat.id)).toEqual([{ message_seq: 1 }, { message_seq: 2 }, { message_seq: 3 }]);
  ...
  it("uses the workspace slug for header-only Chat requests and rejects unknown or conflicting scopes", async () => {
    const store = createStore();
