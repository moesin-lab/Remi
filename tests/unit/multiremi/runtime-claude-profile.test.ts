...
describe("Runtime Claude profiles", () => {
  ...
  it("preserves group configuration and credentials after deleting a Runtime", () => {
    ...
    const profile = store.setRuntimeClaudeProfile(runtime.id, apiProfile, "delete-key")!;
    ...
    expect(store.getRuntimeClaudeProfileKey(runtime.id, profile.credential_id!)).toBeNull();
