...
import { executionGroupProfileOptions, useSetExecutionGroupProfile, type RuntimeClaudeProfile } from "@multiremi/core/runtimes/execution-group-profile";
...
export function ExecutionGroupProviderProfile({ wsId, groupId, canManage, provider }: { wsId: string; groupId: string; canManage: boolean; provider: "codex" | "claude" }) {
  const profileKey = `${provider}_profile` as const;
  const { t } = useT("runtimes");
  const query = useQuery(executionGroupProfileOptions(wsId, groupId));
  return <div className="mx-auto w-full max-w-3xl space-y-5 p-4 sm:p-8">
    <header className="space-y-2">
      <h2 className="text-xl font-semibold">{t($ => $.execution_group.connection_title)}</h2>
      <p className="text-sm leading-6 text-muted-foreground">{t($ => $.execution_group.connection_hint, { group: groupId })}</p>
    </header>
    {query.isPending ? <p role="status">{t($ => $[profileKey].loading)}</p>
      ...
      : <ProfileForm key={`${wsId}:${groupId}:${JSON.stringify(query.data)}`} wsId={wsId} groupId={groupId} canManage={canManage} provider={provider} initial={query.data.profile} />}
  </div>;
}

function ProfileForm({ wsId, groupId, canManage, initial, provider }: { wsId: string; groupId: string; canManage: boolean; initial: RuntimeClaudeProfile | null; provider: "codex" | "claude" }) {
  const profileKey = `${provider}_profile` as const;
  const { t } = useT("runtimes");
  const prefix = useId();
  ...
  const save = useSetExecutionGroupProfile(wsId, groupId);
  ...
  return <form className="space-y-5 rounded-xl border bg-card p-5" onSubmit={event => {
    ...
    if (editable) save.mutate({ profile: enabled ? { ...profile, models: [...new Set([profile.model.trim(), ...models.split(/[,\n]/).map(id => id.trim()).filter(Boolean)])] } : null,
    ...
    {enabled ? <>
      ...
      <div className="space-y-2">
        <label htmlFor={`${prefix}-models`} className="text-sm font-medium">{t($ => $.execution_group.models)}</label>
        <textarea id={`${prefix}-models`} value={models} disabled={!editable} className="min-h-24 w-full rounded-md border bg-background p-2 text-sm" onChange={event => setModels(event.target.value)} />
        <p className="text-sm text-muted-foreground">{t($ => $.execution_group.models_hint)}</p>
      </div>
      <div className="space-y-2">
