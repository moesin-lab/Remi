...
async function setup(overrides: Record<string, unknown> = {}) {
  ...
  const daemon = {
    ...
    recordBotReply: async (...args: unknown[]) => { calls.push({ name: "receipt", args }); },
...
describe("Bot foreground host", () => {
  it("submits attachments and route context through the Bot API and records the real reply", async () => {
    ...
    await meta.onReplyCreated!("platform-card");
    expect(test.calls.at(-1)).toEqual({ name: "receipt", args: ["bot", "binding", "task", "platform-card"] });
