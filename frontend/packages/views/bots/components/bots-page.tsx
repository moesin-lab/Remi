...
import { useCurrentMember } from "@multiremi/core/permissions";
...
function WorkspaceBots({ wsId }: { wsId: string }) {
  ...
  if (selected !== null) return <BotEditor key={selected} canManage={canManage} wsId={wsId} botId={selected === "new" ? null : selected} onBack={() => setSelected(null)} onCreated={bot => setSelected(bot.id)} />;

  return <div className="flex h-full min-h-0 flex-col">
    <PageHeader><h1 className="text-sm font-semibold">{t($ => $.title)}</h1><Button size="sm" className="ml-auto" disabled={!canManage} onClick={() => setSelected("new")}><Plus className="size-4" />{t($ => $.create)}</Button></PageHeader>
      ...
      {bots.isPending ? <p role="status" className="py-8 text-sm text-muted-foreground">{t($ => $.loading)}</p>
        : bots.isError ? <LoadError onRetry={() => void bots.refetch()} />
          : bots.data?.length ? <div className="grid gap-3 md:grid-cols-2">{bots.data.map(bot => <button key={bot.id} onClick={() => setSelected(bot.id)} className="flex min-w-0 flex-col gap-3 rounded-lg border p-4 text-left transition-colors hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            ...
            : <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed px-4 py-16 text-center"><MessageSquare className="size-8 text-muted-foreground" /><p className="text-sm font-medium">{t($ => $.empty)}</p><p className="max-w-md text-sm text-muted-foreground">{t($ => $.empty_hint)}</p><Button disabled={!canManage} onClick={() => setSelected("new")}><Plus className="size-4" />{t($ => $.create)}</Button></div>}
    </div></div>
  </div>;
}

function BotEditor({ wsId, botId, onBack, onCreated, canManage }: { canManage: boolean; wsId: string; botId: string | null; onBack: () => void; onCreated: (bot: Bot) => void }) {
  const { t } = useT("bots");
  ...
  async function save(input: SaveBotInput) {
    if (!canManage) throw new Error(t($ => $.read_only));
    try {
  ...
  async function setEnabled(enabled: boolean) {
    if (!bot || busy || dirty || !canManage) return;
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6"><div className="mx-auto max-w-3xl space-y-6">
      {botId && detail.isPending ? <p role="status" className="text-sm text-muted-foreground">{t($ => $.loading)}</p>
        : botId && detail.isError && !bot ? <LoadError onRetry={() => void detail.refetch()} />
          : <>{botId && detail.isError && bot && <LoadError onRetry={() => void detail.refetch()} />}{bot && <ConnectionStatus bot={bot} />}<BotForm wsId={wsId} bot={bot} busy={busy} readOnly={!canManage} onSave={save} onDirtyChange={setDirty} />{bot && <BotSenders wsId={wsId} bot={bot} canManage={canManage} />}</>}
    </div></div>
...
function BotSenders({ wsId, bot, canManage }: { wsId: string; bot: Bot; canManage: boolean }) {
  const { t } = useT("bots");
