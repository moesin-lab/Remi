...
export function BotForm({ wsId, bot, busy, onSave, onDirtyChange }: {
  wsId: string;
  bot: Bot | null;
  busy: boolean;
  ...
  const unsupported = draft.default_target.kind !== "agent" || draft.platform_bindings.some(binding => binding.platform !== "feishu" || !["feishu", "lark", "bytedance"].includes(binding.domain ?? "feishu")) || rules.some(rule => rule.target.kind !== "agent");
  ...
  const valid = !unsupported && !hasIncompatibleRuntime && (!draft.issue_notifications || (draft.issue_notifications.platform_binding_id && draft.issue_notifications.chat_id.trim() && (!draft.issue_notifications.target || draft.issue_notifications.target.agent_id))) && draft.name.trim() && draft.default_target.agent_id && draft.platform_bindings.length > 0
    && draft.platform_bindings.every(binding => binding.app_id.trim() && binding.host_runtime_id && (binding.app_secret?.trim() || bot?.platform_bindings.some(saved => saved.id === binding.id && saved.app_secret_configured)))
  ...
  return <form id="bot-form" onSubmit={event => { event.preventDefault(); void submit().catch(() => {}); }} className="space-y-6">
    ...
    <fieldset disabled={busy || unsupported} className="space-y-6 disabled:opacity-70">
      ...
      <Button type="submit" disabled={!valid || busy || agents.isPending || runtimes.isPending}>{busy ? t($ => $.saving) : bot ? t($ => $.save) : t($ => $.create)}</Button>
    </fieldset>
  </form>;
}
