...
describe("Feishu workspace membership admission", () => {
  it("isolates monitor policy between Bot accounts sharing a group", async () => {
    const event = messageEvent({ messageId: uniqueMessageId("scoped-policy"), senderOpenId: "ou_sender", chatType: "group", text: "ordinary group message" });
    const { client } = clientWithSender();
    const gate = admission(async () => true);
    setGroupPolicy({ getByChatId: () => ({ monitor: true }) });
    expect(await processFeishuMessageEvent(client, event, "ou_bot", {
      ...gate.options, eventScope: "monitor-bot", groupPolicy: { getByChatId: () => ({ monitor: true }) },
    })).toMatchObject({ monitored: true });
    expect(await processFeishuMessageEvent(client, event, "ou_bot", {
      ...gate.options, eventScope: "mention-bot", groupPolicy: { getByChatId: () => null },
    })).toBeNull();
  });
