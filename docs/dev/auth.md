...
能力组的 Codex / Claude Code 自定义连接由 human 工作区 owner/admin 管理；向已配置连接的组加入 Runtime 同样需要该权限。task token 对新旧配置路由均为 hard deny。API Key 使用 AES-256-GCM 加密与不可变版本，只向绑定机器身份、且属于对应组或正在执行对应冻结任务的 daemon 下发。浏览器响应和任务公共响应不含密钥。迁移、轮换及兼容入口见 [连接契约](../design/acp-codex-via-codex-acp.md#能力组自定义连接)，权限回归见 [execution-group-profile-api.test.ts](../../tests/unit/execution-group-profile-api.test.ts)。
