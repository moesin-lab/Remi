...
const api = vi.hoisted(() => ({ getRuntimeCodexProfile: vi.fn(), setRuntimeCodexProfile: vi.fn() }));
vi.mock("@multiremi/core/api", () => ({ api }));
vi.mock("@multiremi/core/hooks", () => ({ useWorkspaceId: () => "ws-profile" }));
const runtime = { id: "rt-profile", provider: "codex", metadata: { codex_profiles: 1 } } as unknown as AgentRuntime;
const saved = { name: "private", base_url: "https://custom.example/v1", model: "custom-model", env_key: "", auth_mode: "api_key", credential_id: "rck_private" };
afterEach(cleanup);
beforeEach(() => {
  vi.clearAllMocks();
  api.getRuntimeCodexProfile.mockResolvedValue({ profile: null });
  api.setRuntimeCodexProfile.mockImplementation(async (_id, input) => {
    const result = { profile: input.profile ? { ...input.profile, credential_id: "rck_private" } : null };
    api.getRuntimeCodexProfile.mockResolvedValue(result);
    return result;
  });
});
function show(value = runtime, canManage = true) {
  const client = new QueryClient({ defaultOptions: { queries: { retry: false }, mutations: { retry: false } } });
  return render(<RuntimeCodexProfileTab runtime={value} canManage={canManage} />, { wrapper: ({ children }) => <I18nProvider locale="en" resources={{ en: { runtimes: enRuntimes } }}><QueryClientProvider client={client}>{children}</QueryClientProvider></I18nProvider> });
}
...
it("saves the URL, model and key, then clears the password input without exposing the saved value", async () => {
  ...
  await waitFor(() => expect(api.setRuntimeCodexProfile).toHaveBeenCalledWith(runtime.id, { profile: { name: "custom", base_url: saved.base_url, model: saved.model, env_key: "", auth_mode: "api_key" }, api_key: "new-test-key" }));
...
it("keeps an existing key when omitted, supports environment credentials, and restores inheritance", async () => {
  const user = userEvent.setup();
  api.getRuntimeCodexProfile.mockResolvedValue({ profile: saved });
  show();
  await screen.findByLabelText("API key");
  await user.click(screen.getByRole("button", { name: "Save connection" }));
  await waitFor(() => expect(api.setRuntimeCodexProfile).toHaveBeenLastCalledWith(runtime.id, { profile: saved }));
  ...
  await user.click(screen.getByRole("button", { name: "Save connection" }));
  await waitFor(() => expect(api.setRuntimeCodexProfile).toHaveBeenLastCalledWith(runtime.id, { profile: { ...saved, auth_mode: "env", env_key: "REMI_CODEX_API_KEY" } }));
  await waitFor(() => expect(screen.getByRole("switch")).toBeEnabled());
  await user.click(screen.getByRole("switch"));
  await user.click(screen.getByRole("button", { name: "Save connection" }));
  await waitFor(() => expect(api.setRuntimeCodexProfile).toHaveBeenLastCalledWith(runtime.id, { profile: null }));
});

it("retains input on a server error and blocks editing for viewers or old Runtimes", async () => {
  const user = userEvent.setup();
  api.getRuntimeCodexProfile.mockResolvedValue({ profile: saved });
  api.setRuntimeCodexProfile.mockRejectedValue(new Error("Server encryption key is unavailable"));
  const view = show();
  await screen.findByLabelText("API key");
