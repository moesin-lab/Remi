...
export class BotsRepo {
  ...
  private save(id: string, input: SaveBotInput, updating: boolean): Bot {
    ...
    return this.ctx.db.transaction(() => {
      ...
      // Pausing an existing Bot must remain possible after a referenced resource
      // is archived. Only unchanged targets may survive while fully stopped.
      const validateSavedTarget = (target: BotTarget, previous?: BotTarget): void => {
        if (!enabled && previous && botTargetKey(target) === botTargetKey(previous)) return;
        this.validateTarget(workspaceId, target);
      };
      const defaultTarget = resolveBotTarget(input.default_target);
      validateSavedTarget(defaultTarget, current?.default_target);
      ...
      const bindings = input.platform_bindings.map((binding) => {
        ...
        const host = this.ctx.runtimes().getRuntime(hostId);
        const keepsDeletedHostWhileStopped = !enabled && current && existing && !Number(existing.removed)
          && existing.host_runtime_id === hostId && !host;
        if (host?.workspaceId !== workspaceId && !keepsDeletedHostWhileStopped) throw new BotError("Platform host belongs to another workspace");
      ...
      const routes = (input.routes ?? []).map((route) => {
        ...
        const previousRoute = current?.routes.find((candidate) => candidate.id === routeId);
        validateSavedTarget(resolveBotTarget(route.target, defaultTarget),
          previousRoute ? resolveBotTarget(previousRoute.target, current!.default_target) : undefined);
      ...
      if (notifications) {
        ...
        validateSavedTarget(resolveBotTarget(notifications.target ?? defaultTarget, defaultTarget),
          current?.issue_notifications
            ? resolveBotTarget(current.issue_notifications.target ?? current.default_target, current.default_target)
            : undefined);
  ...
  private directive(binding: Row, runtimeId: string): BotDirective {
    ...
    if (running) {
      const states = this.ctx.db.query(`SELECT s.* FROM multiremi_bot_runtime_states s
        JOIN multiremi_bot_platform_bindings p ON p.id = s.platform_binding_id
        WHERE p.platform = ? AND p.domain = ? AND p.app_id = ? AND s.state != 'stopped'
          AND NOT (s.platform_binding_id = ? AND s.runtime_id = ?)`)
        .all(String(binding.platform), String(binding.domain), String(binding.app_id), String(binding.id), runtimeId) as Row[];
  ...
  submitMessage(botId: string, bindingId: string, runtimeId: string, input: SubmitBotMessageInput): SubmitBotMessageResult {
    ...
    const result = this.ctx.db.transaction(() => {
      ...
      if (duplicate) {
        ...
        return { chatSessionId: String(duplicate.chat_session_id), agentId: task.agentId, agentName: this.ctx.agents().getAgent(task.agentId)?.name ?? task.agentId, taskId: task.id, status: task.status, duplicate: true, steered: false, botSessionId: String(duplicate.bot_session_id), senderAllowed: Boolean(this.ctx.db.query("SELECT allowed FROM multiremi_bot_senders WHERE id=?").get(String(duplicate.sender_id))?.allowed) };
      }
      ...
      this.ctx.chat().appendChatMessageWithinTransaction({
        id: messageId, chatSessionId: session.chat_session_id, taskId: task.id,
        role: "user", body, createdAt: now,
      });
      ...
      return { chatSessionId: session.chat_session_id, agentId: task.agentId, agentName: this.ctx.agents().getAgent(task.agentId)?.name ?? task.agentId, taskId: task.id, status: task.status, duplicate: false, steered: Boolean(activeTask), botSessionId: session.id, senderAllowed: sender?.allowed ?? false };
    })();
    if (chatMessageId) {
  ...
  claimOutbound(botId: string, bindingId: string, runtimeId: string, nowInput: Date = new Date()): BotOutboundDelivery | null {
    this.assigned(botId, bindingId, runtimeId);
    return this.ctx.db.transaction((): BotOutboundDelivery | null => {
      ...
      return { id: String(row.id), botId, platformBindingId: bindingId, idempotencyKey: String(row.id), updateMessageId: nullableString(row.update_message_id), chatId: String(row.chat_id), threadId: nullableString(row.thread_id), replyToMessageId: nullableString(row.reply_to_message_id), body: String(row.body), bodyOrigin: row.task_id == null ? "issue" : "agent", claimToken: claim };
    })();
  }
