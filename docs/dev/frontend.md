...
| 飞书账号白名单 | [feishu-sender-allowlist-section.tsx](../../frontend/packages/views/settings/components/feishu-sender-allowlist-section.tsx)、[feishu-bot/queries.ts](../../frontend/packages/core/feishu-bot/queries.ts)、[feishu-bot/mutations.ts](../../frontend/packages/core/feishu-bot/mutations.ts) |
...
集成设置中的飞书账号白名单展示当前机器人自动发现并去重的发送者，提供「待授权」「已允许」状态和加入、移出操作，不查询或关联空间成员。页面按 bot 配置响应的管理权限显示，名单每 10 秒刷新，也支持手动刷新；读取失败单独显示错误。更新成功后仅失效当前工作区的 `feishuBotKeys.senders`，不乐观修改授权状态。账号范围、Issue 创建限制与旧会话处理见[消息接入契约](../feishu-message-ingestion.md#机器人发送者白名单)。
