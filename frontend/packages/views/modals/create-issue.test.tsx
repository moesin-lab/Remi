...
vi.mock("@multiremi/core/issues/stores/quick-create-store", () => ({
...
vi.mock("@multiremi/core/api", async () => {
  ...
  const { parseWithFallback } = await vi.importActual<typeof import("@multiremi/core/api/schema")>(
    "@multiremi/core/api/schema",
  );
  const { DuplicateIssueErrorBodySchema } = await vi.importActual<
    typeof import("@multiremi/core/api/schemas")
  >("@multiremi/core/api/schemas");
  return {
    api: {},
    ApiError,
    parseWithFallback,
...
vi.mock("@multiremi/ui/components/ui/switch", () => ({
  Switch: ({
    checked,
    onCheckedChange,
  }: {
...
vi.mock("@multiremi/ui/components/common/file-upload-button", () => ({
