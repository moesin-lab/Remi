...
| Chat 页面、会话管理与队列 | [Chat 契约](chat.md)、[Chat 视图](../frontend/packages/views/chat)、[ChatRepo](../packages/server/src/store/repos/chat-repo.ts) | 创建者私聊；运行中追加串行执行，立即处理取消当前运行；归档后只读，恢复后可继续 | [队列测试](../tests/unit/multiremi/chat-queue.test.ts)、[浏览器冒烟](../tests/integration/smoke-chat-workspace.ts) |
