...
import { agentListOptions, squadListOptions } from "@multiremi/core/workspace/queries";
...
export function AgentCreatePanel({
  ...
  // Dead-end guard: agent mode with no usable runtime (no agent at all, the
  // runtime never reported a CLI version, or the version is too old) leaves
  // Create permanently disabled — a fresh install hits this on the very first
  // `c`. Once the queries have actually resolved, fall back to the manual
  // form instead of presenting a dead dialog. One-shot per open; the user can
  // still switch back to agent mode by hand.
  const autoFellBack = useRef(false);
  useEffect(() => {
    if (autoFellBack.current) return;
    if (!agentsLoaded || !runtimesLoaded || !versionBlocked) return;
