...
describe("BotForm", () => {
  ...
  it.each([false, true])("keeps an inactive saved account editable after its secret was cleared (bot enabled: %s)", async (enabled) => {
    const user = userEvent.setup();
    renderForm(botFixture({ enabled, platform_bindings: [{ ...binding(), enabled: false, app_secret_configured: false, app_secret_hint: null }] }));
    await waitFor(() => expect(screen.getByRole("button", { name: "Save changes" })).toBeEnabled());
    await user.type(screen.getByLabelText("Bot name"), " updated");
    await user.click(screen.getByRole("button", { name: "Save changes" }));
    await waitFor(() => expect(save).toHaveBeenCalledTimes(1));
    expect(save.mock.calls[0]![0].platform_bindings[0]).toMatchObject({ app_secret_op: "keep", app_secret: undefined });
  });

  it("blocks editing an unsupported notification target without losing it", async () => {
    const bot = botFixture({ issue_notifications: { platform_binding_id: "binding-1", chat_id: "group-1", target: JSON.parse('{"kind":"future","agent_id":"agent-1"}') } });
    renderForm(bot);
    expect(screen.getByRole("alert")).toHaveTextContent("This bot uses configuration that this version cannot edit.");
    expect(screen.getByRole("button", { name: "Save changes" })).toBeDisabled();
