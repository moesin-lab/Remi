...
Bot 的执行过程沿用飞书原生 CoT 和交互卡片，最终回复统一通过持久化 outbox 发送；过程展示不另外发送最终结果，避免流式处理与投递队列重复回复。出站发送使用稳定幂等键，并保存平台回执以关联后续回复。若已有投递记录指定了原卡片回执，则更新该卡片；更新失败保持失败/重试状态，不改成新发另一条消息。原生过程展示、卡片更新与新消息发送仍需真实平台验证。 Bot 的执行机器与连接宿主可以不同，因此出站图片不读取连接宿主的本地文件路径；只使用平台图片引用或远程图片，执行机器上的本地图片尚未提供跨 Runtime 上传链路。
...
Web 中普通成员可查看 Bot 配置和已发现账号；空间 owner/admin 可创建、修改、启停、删除 Bot，以及管理白名单。
