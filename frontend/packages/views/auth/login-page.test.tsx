...
vi.mock("@multiremi/core/api", () => ({
  api: {
    ...
    getLarkLoginUrl: mockApiGetLarkLoginUrl,
  },
}));

vi.mock("@multiremi/core/types", () => ({}));
...
// The email OTP flow is retained ONLY for the CLI browser handoff. Passing
// cliCallback is what makes the login page render the email form at all;
// regular (non-CLI) web login is Feishu-only.
const CLI_CB = { url: "http://localhost:9876/callback", state: "abc" };
...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to Multiremi' title", () => {
    ...
    expect(
      screen.getByText(/sign in to multiremi/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("non-CLI web login is Feishu-only (no email form)", () => {
    renderWithI18n(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByRole("button", { name: /sign in with feishu/i }),
    ).toBeInTheDocument();
  ...
  it("CLI mode: verifyCode obtains a token and redirects to the cli_callback", async () => {
    mockSendCode.mockResolvedValueOnce(undefined);
    // CLI handoff uses api.verifyCode directly (returns the bearer token) and
    // redirects to the localhost callback — it does NOT seed workspaces / call
    // onSuccess (that is the web-session path, which is Feishu-only now).
    mockApiVerifyCode.mockResolvedValueOnce({ token: "jwt-xyz" });
    ...
    await waitFor(() => {
      ...
      expect(window.location.href).toContain("http://localhost:9876/callback");
      expect(window.location.href).toContain("jwt-xyz");
    });
  });

  it("shows error on invalid code", async () => {
    mockSendCode.mockResolvedValueOnce(undefined);
    mockApiVerifyCode.mockRejectedValueOnce(new Error("Invalid code"));
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("multimira_token", "existing-jwt");
  ...
  it("CLI mode: calls onTokenObtained after successful verification", async () => {
    mockSendCode.mockResolvedValueOnce(undefined);
    mockApiVerifyCode.mockResolvedValueOnce({ token: "jwt-xyz" });
    const onTokenObtained = vi.fn();
