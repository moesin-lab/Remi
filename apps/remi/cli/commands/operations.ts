...
function runtimeSpecs(): CommandSpec[] {
  ...
  return [
    ...
    op({ id: "runtime.group.profile.get", path: ["runtime", "group", "profile", "get"], description: "Get an execution group's custom provider connection", method: "GET", apiPath: i => `/api/execution-groups/${encodePath(positional(i, 0, "group"))}/provider-profile`, auth: HUMAN, positionals: [ref("group")], query: i => ({ workspace_id: requiredWorkspace(i) }) }),
    op({ id: "runtime.group.profile.set", path: ["runtime", "group", "profile", "set"], description: "Set the group connection and model allowlist with --file; profile: null restores the workspace gateway", method: "PUT", apiPath: i => `/api/execution-groups/${encodePath(positional(i, 0, "group"))}/provider-profile`, auth: HUMAN, mutation: "write", positionals: [ref("group")], options: INPUT_OPTIONS, query: i => ({ workspace_id: requiredWorkspace(i) }) }),
    op({ id: "runtime.codex-profile.get", path: ["runtime", "codex-profile", "get"], description: "Compatibility: get the Runtime's execution-group Codex connection", method: "GET", apiPath: runtime("/codex-profile"), auth: HUMAN_DAEMON, positionals: [ref("runtime")] }),
    op({ id: "runtime.codex-profile.set", path: ["runtime", "codex-profile", "set"], description: "Compatibility: set the Runtime's group connection as a workspace admin; prefer runtime group profile set", method: "PUT", apiPath: runtime("/codex-profile"), mutation: "write", auth: HUMAN, positionals: [ref("runtime")], options: INPUT_OPTIONS }),
    op({ id: "runtime.claude-profile.get", path: ["runtime", "claude-profile", "get"], description: "Compatibility: get the Runtime's execution-group Claude connection", method: "GET", apiPath: runtime("/claude-profile"), auth: HUMAN_DAEMON, positionals: [ref("runtime")] }),
    op({ id: "runtime.claude-profile.set", path: ["runtime", "claude-profile", "set"], description: "Compatibility: set the Runtime's group connection as a workspace admin; prefer runtime group profile set", method: "PUT", apiPath: runtime("/claude-profile"), mutation: "write", auth: HUMAN, positionals: [ref("runtime")], options: INPUT_OPTIONS }),
    ...
    op({ id: "runtime.model.catalog", path: ["runtime", "model", "catalog"], description: "List model providers for the fleet or an execution target", method: "GET", apiPath: "/api/models", auth: HUMAN, options: [{ name: "runtime", type: "string", valueName: "runtime-id", description: "Filter by legacy Runtime target", conflictsWith: ["execution-group"] }, { name: "execution-group", type: "string", valueName: "group-id", description: "Filter by execution group", conflictsWith: ["runtime"] }], query: (i) => queryOptions(i, { runtime_id: stringOption(i, "runtime"), execution_group_id: stringOption(i, "execution-group") }), collections: ["providers"] }),
