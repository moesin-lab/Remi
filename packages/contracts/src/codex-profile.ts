/** A Runtime-owned Responses connection. Plaintext credentials are never part of this configuration. */
export interface RuntimeCodexProfile {
  name: string;
  base_url: string;
  model: string;
  env_key: string;
...
export interface RuntimeCodexProfileConfig {
  profile: RuntimeCodexProfile | null;
}

export interface RuntimeCodexProfileInput extends RuntimeCodexProfileConfig {
...
export function parseRuntimeCodexProfile(value: unknown): RuntimeCodexProfile | null {
  ...
  if (auth_mode === "env" && !/^REMI_CODEX_[A-Z0-9_]+$/.test(env_key)) throw new Error("env_key must name a REMI_CODEX_* Runtime environment variable");
  let url: URL;
