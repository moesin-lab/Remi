...
export function botCommandSpecs(): CommandSpec[] {
  return [
    ...
    },
    {
      id: "bot.sender",
      path: ["bot", "sender"],
      description: "Manage discovered bot senders and their allowlist status",
      parse: "passthrough",
      run: async () => { throw new CliError("usage", "usage: remi bot sender list|allow|revoke ..."); },
    },
    {
      id: "bot.session",
      path: ["bot", "session"],
      description: "Inspect bot conversations and their execution targets",
      parse: "passthrough",
      run: async () => { throw new CliError("usage", "usage: remi bot session list <bot>"); },
    },
